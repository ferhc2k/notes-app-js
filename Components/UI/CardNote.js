...
export const CardNote = (data) => {
    let currentPage = location.hash.split("/")[1] || "notes";
    ...
    card.innerHTML = `
        ...
        <p>Ultima actualización: ${moment(moment(data.date).format()).fromNow()}.</p>
    `;
    card.appendChild(setCardsAction[currentPage](data.id));
    return card;
}